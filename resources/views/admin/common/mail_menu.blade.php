<div class="box">
	<div class="box-body no-padding d-block">
		<ul class="nav nav-pills nav-stacked d-flex flex-column">
			...
			@endif
			@if(Helpers::has_permission(Auth::guard('admin')->user()->id, 'manage_email_template'))
				<li {{ isset($list_menu) &&  $list_menu == 'menu-12' ? 'class = active' : '' }}><a href="{{ url("admin/email-template/12") }}">Assign to site manager</a></li>
			@endif
			@if(Helpers::has_permission(Auth::guard('admin')->user()->id, 'manage_email_template'))
				<li {{ isset($list_menu) &&  $list_menu == 'menu-13' ? 'class = active' : '' }}><a href="{{ url("admin/email-template/13") }}">Payment Verification</a></li>
			@endif
		</ul>
	</div>
</div>