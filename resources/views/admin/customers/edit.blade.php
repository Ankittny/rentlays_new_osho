                    ...
                    <div class="box">
                        ...
                        <form class="form-horizontal" action="{{ url('admin/edit-customer') }}/{{ $user->id }}"
                            ...
                            <div class="box-body">
                                ...
                                <div class="form-group row mt-3 ">
                                    <label for="exampleInputPassword1" class="control-label col-sm-3 fw-bold text-md-end mb-2 mb-md-0">Country<span class="text-danger">*</span></label>
                                    <div class="col-sm-5">
                                        <select class="form-control" name="country">
                                           @if($country->count()>0)
                                            @foreach($country as $items) 
                                            <option value="{{$items->name}}" <?php if($items->name == $user->country)echo 'selected'; ?>>{{ucfirst($items->name)}}</option>
                                            @endforeach
                                           @endif
                                        </select>
