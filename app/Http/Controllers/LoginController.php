...
use App\Models\{Favourite, PasswordResets, Settings, User, UserDetails, UsersVerification,Country,PmsJobApproval};
...
class LoginController extends Controller
{
    ...
    public function authenticate(Request $request, EmailController $email_controller)
    {
        ...
        if ($validator->fails()) {
            ...
            if (!empty($users)) {
                if ($users->status != 'Inactive') {
                    ...
                    // add by ankit 06/10/2024
                    if (Auth::attempt(['email' => $request->email, 'password' => $request->password], $remember)) {
                        $email_controller->welcome_email($users);
                        self::addFavourite();
                        Auth::logout();
    ...
    public function addFavourite()
    {
        if (Session::has('favourite_property')) {
            ...
            $property_id = Session::get('favourite_property');
    ...
    {       
            ...
            if ($password_resets->count()) {
                ...
                $data['result']     = User::whereEmail($password_result->email)->first();
                $data['token']      = $request->secret;
                $data['get_job_id'] = PmsJobApproval::whereIn('status', ['pending', 'approved'])->where(['user_id'=>$data['result']->id])->first();
               if(empty($data['get_job_id'])){
                  abort(404);
                } else {
                    return view('jobapproval.pms_approval',$data);
                }
        }
    }
