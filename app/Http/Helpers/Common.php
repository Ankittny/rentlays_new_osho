    ...
    public static function one_time_message($class, $message)
{
    ...
    // Debugging
    \Log::info('Flash message set: ' . $message);
}
    ...
    public static function getPrice($propertyId, $checkIn, $checkOut, $guestCount, $force = false)
    {
        ...
        $dates = self::get_days($date1, $date2);
        ...
        $result['property_default'] = [
            ...
            'local_to_propertyRate' => self::currency_rate($property->property_price->currency_code, self::getCurrentCurrencyCode()),
        ];
        ...
        foreach ($dates as $date) {
            if (in_array($date, array_keys($differentPriceDatePrice))) {
                $differentPrice += self::convert_currency($property->property_price->currency_code, '', $differentPriceDatePrice[$date]);
            } elseif (date('N', strtotime($date)) == 5 && $property->property_price->weekend_price != 0) {
                $weekendPrice += $property->property_price->weekend_price;
            } elseif (date('N', strtotime($date)) == 6 && $property->property_price->weekend_price != 0) {
                $weekendPrice += $property->property_price->weekend_price;
            } else {
            ...
            } elseif (($countDays - ($countMonths * 28)) % 7 == 0 && $property->property_price->weekly_discount != 0) {
                $countWeeks++;
                $temp_total = $differentPrice + $weekendPrice + $nightPrice;
        ...
        foreach ($dates as $key => $value) {
            ...
            if (in_array($value, array_keys($differentPriceDatePrice))) {
                $allDate[$key]['price'] = moneyFormat($symbol, numberFormat(self::convert_currency($property->property_price->currency_code, '', $differentPriceDatePrice[$value]), $decimals));
                $allDate[$key]['original_price'] = self::convert_currency($property->property_price->currency_code, '', $differentPriceDatePrice[$value]);

            } elseif (date('N', strtotime($value)) == 5 && $property->property_price->weekend_price != 0) {
            ...
            } elseif (date('N', strtotime($value)) == 6 && $property->property_price->weekend_price != 0) {
